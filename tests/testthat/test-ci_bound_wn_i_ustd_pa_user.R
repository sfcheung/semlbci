...
time2l <- system.time(out2l <- ci_bound_wn_i(7, 5, sem_out = fit, f_constr = fn_constr0, which = "lbound", opts = opts0, verbose = TRUE, ciperc = ciperc))
...
test_out2l <- test_constr(fit = fit, dat = simple_med, ciperc = ciperc, parc = "asq == ", modc0 = modc0, ci_out = out2l, semfct = lavaan::sem, tol = 1e-3, fixed.x = FALSE)
