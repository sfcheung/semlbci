...
time2l <- system.time(out2l <- ci_bound_wn_i(10,16, sem_out = fit, f_constr = fn_constr0, which = "lbound", opts = opts0, verbose = TRUE, ciperc = ciperc))
...
y ~ c(b1, b2)*m
"
fit_tmp <- lavaan::sem(modc0, simple_med_mg, fixed.x = FALSE, group = "gp")
