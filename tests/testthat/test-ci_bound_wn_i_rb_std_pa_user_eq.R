...
time1u <- system.time(out1u <- ci_bound_wn_i(6, 5, sem_out = fit, f_constr = fn_constr0, which = "ubound", opts = opts0, verbose = TRUE, ciperc = ciperc, standardized = TRUE, sf = sf1$c_r, wald_ci_start = FALSE, std_method = "internal"))
time2l <- system.time(out2l <- ci_bound_wn_i(1, 5, sem_out = fit, f_constr = fn_constr0, which = "lbound", opts = opts0, verbose = TRUE, ciperc = ciperc, standardized = TRUE, sf = sf2$c_r, wald_ci_start = FALSE, std_method = "internal"))
...
save(fitc_out1l, fitc_out1u,
     ...
     geteststd1,
     geteststd2,
     file = "inst/testdata/test-ci_bound_wn_i_rb_std_pa_user_eq.RData",
     compress = "xz",
     compression_level = 9)
}

load(system.file("testdata", "test-ci_bound_wn_i_rb_std_pa_user_eq.RData",
                  package = "semlbci"))
