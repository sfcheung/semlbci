...
mod_OpenMx <- mxModel(
  ...
  mx.algebras = list(f1213 = mxAlgebra(f12 * f13, name = "f1213")),
...
ci_lavaan <- matrix(
              c(out19l, out23l, out25l,
                out19u, out23u, out25u), 3, 2)

# OpenMx could not find one of the limits.

test_that("Equal to OpenMx LBCI", {
    expect_equal(
        as.vector(ci_lavaan)[-4], 
        unlist(ci_OpenMx[c(3:5), c("lbound", "ubound")])[-4],
        tolerance = 1e-6,
        ignore_attr = TRUE
      )
  })
