...
sf2 <- scaling_factor2(fit, 10, standardized = TRUE)
...
time2l <- system.time(out2l <- ci_bound_wn_i(10, 16, sem_out = fit, f_constr = fn_constr0, which = "lbound", opts = opts0, verbose = TRUE, ciperc = ciperc, sf = sf2$c_r, sf2 = sf2$c_rb, standardized = TRUE, wald_ci_start = FALSE, std_method = "internal"))
...
save(fitc_out1l, fitc_out1u,
     ...
     geteststd1,
     geteststd2,
     file = "inst/testdata/test-ci_bound_wn_i_mg_rb_std_pa.RData",
     compress = "xz",
     compression_level = 9)
}

load(system.file("testdata", "test-ci_bound_wn_i_mg_rb_std_pa.RData",
                  package = "semlbci"))
