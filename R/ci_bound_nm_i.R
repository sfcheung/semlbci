...
ci_bound_nm_i <- function(i = NULL,
    ...
    if (test_generic) {
        ...
        # TO-FIX:
        # For unstandardized solution, i_depend is the position in the
        # parameter table.
        # For standardized soluton, i_dpeend is the position in the free
        # paraemter vector.
        # This problem was overlooed because they happened to be the same
        # in some models.
        ...
        if (sem_out@Model@eq.constraints) {
            ...
            i_in_eq <- apply(eq_K, 1,
                              function(x) any(sort(unique(x)) != c(0, 1)))
            i_in_eq_label <- p_table[which(i_in_eq), "plabel"]
            npar_reduced <- ncol(eq_K)
            i_depend_reduced <- which(apply(
              eq_K[i_depend_free, seq_len(npar_reduced), drop = FALSE] != 0, 2,
              any))
          } else {
            eq_K  <- NULL
            eq_k0 <- NULL
        ...
        if (wald_ci_start) {
            ...
            xstart0[i_exp_target, "lbcistart"] <-
                      xstart0[i_exp_source, "lbcistart"]
        ...
        lbci_b_f <- function(param_depend, sem_out, debug, lav_warn) {
            ...
            if (!is.null(eq_K)) {
                ...
                param_i <-
                  as.numeric(eq_K %*% matrix(param_i_reduced, npar_reduced, 1) +
                             eq_k0)
            ...
            if (is.null(attr(param_depend, "refit"))) {
                ...
                start1[i_exp_target, "free"] <-
                                    start1[i_exp_source, "free"]
                start1[i_exp_target, "ustart"] <-
                                    start1[i_exp_source, "ustart"]
